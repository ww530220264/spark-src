...
object RDD_2_Api {
  ...
  def mapPartitions(spark: SparkSession): Unit = {
    val rdd = spark.sparkContext.parallelize(Seq(1, 2, 3, 4, 5, 6, 7), 3)
    val newRdd1 = rdd.mapPartitions(iterable => {
      var arr = List[String]()
      iterable.foreach(ele => {
        println(Thread.currentThread().getName + "--" + ele)
        arr :+= (ele + "---" + Thread.currentThread().getName)
      })
      arr.iterator
    })
