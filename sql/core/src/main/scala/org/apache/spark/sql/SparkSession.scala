...
object SparkSession extends Logging {
  ...
  class Builder extends Logging {
     ...
     * 获取SparkSession 首先从ThreadLocal中获取，如果没有检查是否存在全局SparkSession，
     * 如果没有则根据创建一个新的SparkSession并赋值为全局SparkSession
     * In case an existing SparkSession is returned, the config options specified in
     * this builder will be applied to the existing SparkSession.
     * 如果已经存在SparkSession，则传入的配置选项将被应用到存在的SparkSession
     * @since 2.0.0
     */
    def getOrCreate(): SparkSession = synchronized {
      // SparkSession只能在Driver端创建和访问
      assertOnDriver()
      // Get the session from current thread's active session.//首先从本地线程变量中获取
      ...
      SparkSession.synchronized {
        ...
        // 重新创建一个SparkSession,首先创建SparkContext
